...
import net.tekpartner.code.sample.coherence.pof.FilterFetcher;

/**
 * FilterFactory is a utility class that provides a set of factory methods that
 * is used for building instances of {@link Filter} or {@link ValueExtractor}.
 * This example uses an InvocationService to build the Filters and
 * ValueExtractors which requires a network round trip. This class exists for
 * compatibility with C++ and .Net and as a java example of using the
 * InvocationService.
 * <p/>
 * The FilterFactory api accepts a String that specifies the creation of rich
 * Filters in a format that should be familiar to anyone that understands SQL
 * WHERE clauses. For example the Sring "street = 'Main' and state = 'TX'" would
 * create a tree of Filters that is the same as the following Java code:
 * 
 * new AndFilter( new EqualsFilter("getStreet","Main"), new
 * EqualsFilter("getState","TX"));
 * 
 * @see QueryHelper <p/>
...
public class FilterFactory {
	...
	/**
	 * Construct a new FilterFactory and use the given InvocationService name if
	 * the implementation needs it. C++ and .Net platforms need to use an
	 * InvocationService to go over the network to create Filters and
	 * ValueExtractors
	 * 
	 * @param sServiceName
	 *            The name of the InvocationService to use
	 */
	public FilterFactory(String sServiceName) {
		m_sServiceName = sServiceName;
		m_service = null;
		/*
		 * It isn't necessary to use the InvocationService under java as
		 * QueryHelper is supported. Un-comment the section below if you would
		 * like to see the InvocationService in action.
		 */

		/*
		 * try { m_service = (InvocationService)
		 * CacheFactory.getService(sServiceName); } catch (Exception e) {
		 * System.out.println("\n Exception hooking up to service: " +
		 * sServiceName); System.out.println(e); m_service = null; }
		 */
	 ...
	 * 
	 * @param s
	 *            the Coherence Query Language string representing a Filter
	 * 
	...
	public Filter createFilter(String s) {
		return m_service == null ? QueryHelper.createFilter(s) : fetchFilter(s,
				null);
	}
	 ...
	 * 
	 * @param s
	 *            a String in the Coherence Query Language representing a Filter
	 * @param aBindings
	 ...
	 *            a String in the Coherence Query Language representing a Filter
	 * @param mapBindings
	 *            the Map of Objects to use for Bind variables
	 * 
	 * @return the constructed Filter
	 * 
	 * @throws FilterBuildingException
	 ...
	 * 
	 * @param s
	 *            a String in the Coherence Query Language representing a
	 *            ValueExtractor
	 * 
	 ...
	 *            the array of Objects to use for Bind variables
	 * 
	 * @return the constructed Filter or null depending on member status (@see
	 *         InvocationService).
	 */
	...
	/**
	 * An Object that is the result of each classified dispatch as the tree of
	 * Objects is built
	 */
