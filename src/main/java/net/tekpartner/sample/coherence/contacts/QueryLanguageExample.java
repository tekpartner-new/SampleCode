 ...
 * <p/>
 * The purpose of this example is to show how to create Extractors and Filters
 * by using the Coherence QueryHelper interface indirectly through a
 * FilterFactory. This example does the same operations as the QueryExample in
...
public class QueryLanguageExample {
	 ...
	 *            cache to query
	 * @param ff
	 *            the FilterFactory to use for making Filters and
	 *            ValueExtractors
	 */
	public void query(NamedCache cache, FilterFactory ff) {
		...
		setResults = cache
				.entrySet(ff
						.createFilter("lastName like 'S%' and homeAddress.state = 'MA'"));
		...
		System.out.println("count > "
				+ nAge
				+ ": "
				+ cache.aggregate(ff.createFilter("age > ?1", aEnv),
						new Count()));
